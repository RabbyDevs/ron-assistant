use super::{CONFIG, Context, Error, FromStr, helper};
...
pub async fn getinfo(
    ctx: Context<'_>,
    #[description = "Users for the command, accepts Discord ids, ROBLOX users and ROBLOX ids."]
    users: String,
    #[description = "How many badge pages should the command get?"] badge_max_iterations: Option<
        i64,
    >,
) -> Result<(), Error> {
    ...
    let (roblox_ids, roblox_conversion_errors) =
        helper::merge_types(&ctx.data().reqwest_client, &ctx.data().rbx_client, users).await;

    if !roblox_conversion_errors.is_empty() {
        ctx.channel_id()
            .say(&ctx.http(), &roblox_conversion_errors.join("\n"))
            .await?;
    }
    ...
    for id in roblox_ids {
        let badge_data_future =
            helper::badge_data(&ctx.data().reqwest_client, id.clone(), badge_iterations);
        ...
        let user_details = ctx
            .data()
            .rbx_client
            .user_details(id.parse::<u64>().expect("Invalid user ID"))
            .await?;
        let created_at: DateTime<Local> =
            DateTime::from_str(&user_details.created_at).expect("Invalid date");
        let avatar_image =
            helper::get_roblox_avatar_bust(&ctx.data().reqwest_client, user_details.id.to_string())
                .await;
        let associated_discord_ids = helper::roblox_id_to_discord_ids(
            &ctx.data().reqwest_client,
            user_details.id.to_string(),
        );

        ctx.channel_id().say(&ctx.http(), "### Username").await?;
        ctx.channel_id()
            .say(&ctx.http(), user_details.username.to_string())
            .await?;
        ctx.channel_id().say(&ctx.http(), "### User ID").await?;
        ctx.channel_id()
            .say(&ctx.http(), format!("{}", user_details.id))
            .await?;
        // Normal behavior: Send full info with badge count, etc.
        let mut embed = helper::new_embed_from_template(ctx.data())
            .await
            ...
            .field(
                "User Link",
                format!("https://roblox.com/users/{}", user_details.id),
                true,
            );

        let sanitized_description = new_line_regex
            .replace(&user_details.description, "")
            .into_owned();
        let created_at_timestamp = created_at.timestamp();

        let account_age = SystemTime::now().duration_since(UNIX_EPOCH)?
            - Duration::from_secs(created_at_timestamp as u64);
        ...
        embed = embed
            ...
            .field(
                "Account Creation",
                format!("<t:{}:D>{}", created_at_timestamp, new_account_message),
                false,
            );
        ...
        if let Ok(discord_ids) = associated_discord_ids.await {
            embed = embed.field(
